...
function load(searchCriteria) {
  ...
  searchCriteria.fields.forEach(function(field) {
    ...
    if (searchCriteria.options.populate && _find(searchCriteria.options.populate, {path: field.path}) && (field.ref || field.base)) {
      // populate only if really need
      addPopulate(searchCriteria, field);
    }
...
function addPopulate(searchCriteria, field) {
  ...
  if (index >= searchCriteria.populate.length) {
    populate = { path: path, sort: {}, model: field.ref };
    if (field.base) {
      populate.select = select;
    }
